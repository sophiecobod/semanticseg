...
parser.add_argument('--figChart', type=float, default=0, help='probability that a figure is a chart (or others)')
...
# lists
fig_list = np.genfromtxt(args.pathFig + args.listFig, delimiter='*', dtype=None, encoding=None)
chart_list = np.genfromtxt(args.pathFig + args.listChart, delimiter='*', dtype=None, encoding=None)
table_list = np.genfromtxt(args.pathTable + args.listTable, delimiter='*', dtype=None, encoding=None)
text_list = np.genfromtxt(args.pathText + args.listText, delimiter='*', dtype=None, encoding=None)
...
for it_page in range(args.pageNum):
    print(it_page)
    #try:
    fout_name = '{}{}.tex'.format(args.prefix, it_page) #nom du fichier tex ecrit 
    ...
    for it_column in range(num_columns):
        ...
        while offset_y < (args.pageHeight - marginPt):
            ...
            if False:#opt < args.distribution['figure']:
                if False: #np.random.random() > args.figChart:
                    # image caption
                ...
                if img_caption == '':
                    ...
                    offset_y = offset_y_old
                    offset_y += max(img_height, caption_height) + font_size * args.paraSkipRatio
            elif False:#opt < args.distribution['table']:
                # copied from above
                # image caption
                img_caption = ''
                textfile = np.random.choice(text_list)
